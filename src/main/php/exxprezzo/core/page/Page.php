...
final class Page extends AbstractOutput {
	...
	public function __construct($outputObject) {
		...
		$stmt = $dbh->prepare('SELECT `pageId`, `preferredFunctionTemplate`, `theme`, `name`, `defaultBox` FROM `page`
				...
				LIMIT 1');
		$instanceId = $this->main->getSource()->getInstanceId();
		$stmt->bindParam(':moduleInstanceId', $instanceId);
		$function = $this->main->getSource()->getFunctionName();
		...
		if ($stmt->execute() && $layoutEntry = $stmt->fetch()) {
			...
			$this->templateName = $layoutEntry['preferredFunctionTemplate']
					...
					: $this->main->getSource()->getFunctionName()
				;
			$this->layout = array(
	...
	public function getContent() {
		...
		if ($widgetOutput instanceof ContentOutput)
			$widgetOutput->setTemplate(static::getTemplate('module', $this->layout['theme'], array($module->getName(), $this->templateName)));
		...
		foreach($this->widgets as $box => $widgets) foreach($widgets as $widget) {
			if ($widget instanceof ContentOutput)
				$widget['output']->setTemplate(static::getTemplate('module', $this->layout['theme'], array($widget['module']->getName(), $widget['template'])));
